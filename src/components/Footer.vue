<script setup>

</script>

<template>
    <footer class="bg-[#F3E5D9]">
      <div>
        <div class="w-5/6 mx-auto flex justify-between py-10">
          <div class="space-y-4">
            <img src="/images/logo/logo.svg" alt="Logo">
            <div class="w-full py-3 px-4 border border-[#141415] bg-transparent flex gap-2">
              <span class="font-medium text-[#0A3B5A]">English</span>
              <img src="/images/icons/dropdown.svg" alt="Dropdown">
            </div>
          </div>

          <div class="text-[#141415]">
            <h1 class="font-semibold text-xl">About</h1>
            <ul class="py-4 space-y-2 text-base">
              <li>
                <a href="#">Our Story</a>
              </li>
              <li>
                <a href="#">Blog</a>
              </li>
              <li>
                <a href="#">Legal</a>
              </li>
              <li>
                <a href="#">FAQs</a>
              </li>
              <li>
                <a href="#">Contact us</a>
              </li>
            </ul>
          </div>

          <div class="text-[#141415]">
            <h1 class="font-semibold text-xl">Product</h1>
            <ul class="py-4 space-y-2 text-base">
              <li>
                <a href="#">Men</a>
              </li>
              <li>
                <a href="#">Women</a>
              </li>
              <li>
                <a href="#">Sales</a>
              </li>
              <li>
                <a href="#">New in</a>
              </li>
            </ul>
          </div>

          <div class="text-[#141415]">
            <h1 class="font-semibold text-xl">Connect with us</h1>
            <ul class="flex space-x-5 py-5">
              <li>
                <a href="#">
                  <img class="w-full h-full object-fit" src="/images/socials/twitter.svg" alt="Twiiter">
                </a>
              </li>
              <li>
                <a href="#">
                  <img class="w-full h-full object-fit" src="/images/socials/facebook.svg" alt="Facebook">
                </a>
              </li>
              <li>
                <a href="#">
                  <img class="w-full h-full object-fit" src="/images/socials/instagram.svg" alt="Instagram">
                </a>
              </li>
            </ul>
          </div>
        </div>
        <hr class="border border-[#82776D]">
        <div class="text-[#141415] text-xs font-medium text-center py-5">
          <p>2023 FASHION FIT All right reserved. Designed by <a href="https://twitter.com/AvoDesigns_" target="_blank" class="underline">Adenike Abioye</a></p>
          <p>Developed with TailwindCSS, Vite and &#10084; by <a href="https://github.com/codejutsu1" target="_blank" class="underline">Codejutsu</a>.</p>
        </div>
      </div>
    </footer>
</template>